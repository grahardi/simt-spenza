...
@section('title', 'Roles - ' . $karyawan->nama)

@section('content')
@if (!$member)
    <div class="card" style="max-width:480px;">
        <div class="card-header"><h3 class="card-title">Buat Akun Login untuk {{ $karyawan->nama }}</h3></div>
        <div class="card-body">
            <p class="text-muted small">Karyawan ini belum punya akun login (belum ada di tabel <code>member</code>).</p>
            <form method="POST" action="{{ route('superadmin.karyawan.buat-akun', $karyawan) }}">
                @csrf
...
@else
    <div class="card mb-3">
        <div class="card-header"><h3 class="card-title">Roles - {{ $karyawan->nama }} (Login: {{ $member->user }})</h3></div>
        <div class="card-body">
            <form method="POST" action="{{ route('superadmin.karyawan.roles.simpan', $karyawan) }}">
                @csrf
                <div class="form-group">
                ...
                <a href="{{ route('superadmin.karyawan.index') }}" class="btn btn-outline-secondary">Kembali</a>
            </form>
        </div>
    </div>
    ...
    <div class="card" style="max-width:480px;">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('superadmin.karyawan.reset-password', $karyawan) }}">
                @csrf
                <div class="form-group">
